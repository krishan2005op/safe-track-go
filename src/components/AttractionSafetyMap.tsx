...
export const AttractionSafetyMap = ({
  onAttractionSelect,
}: AttractionSafetyMapProps) => {
  const [hoveredAttraction, setHoveredAttraction] = useState<string | null>(
    null
  );
  const navigate = useNavigate();
  ...
  const attractions: Attraction[] = [
    {
      ...
      status: "Safe",
      description:
        "The Red Fort is a historic fort in Delhi and a UNESCO World Heritage Site, known for its stunning Mughal architecture.",
    ...
    },
    {
      ...
      status: "Caution",
      description:
        "Qutub Minar is a UNESCO World Heritage Site and the tallest brick minaret in the world, located in Delhi.",
    ...
    },
    {
      ...
      status: "Safe",
      description:
        "India Gate is a war memorial located in New Delhi, dedicated to the soldiers who died during World War I.",
    ...
    },
    {
      ...
      status: "Safe",
      description:
        "The Lotus Temple is a Baháʼí House of Worship, famous for its flowerlike shape and peaceful ambiance.",
      link: "https://en.wikipedia.org/wiki/Lotus_Temple",
    },
  ];

  const handleBookTicket = (attraction: string) => {
    onAttractionSelect(attraction);
    navigate("/booking");
  };

  return (
    <div className="w-full h-[600px] bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl p-6 shadow-lg relative overflow-hidden">
      {/* Map Area */}
      <div className="relative w-full h-full rounded-lg bg-white shadow-inner border border-gray-200">
        {/* Current Location */}
        <div
          className="absolute w-5 h-5 bg-blue-600 rounded-full border-2 border-white shadow-md"
          style={{
            left: `${currentLocation.x}%`,
            top: `${currentLocation.y}%`,
            transform: "translate(-50%, -50%)",
        ...
        >
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 text-xs font-medium text-blue-700 whitespace-nowrap">
        ...
        {attractions.map((attraction) => (
          ...
          >
            <div
              className={`w-6 h-6 rounded-full border-2 shadow-md flex items-center justify-center ${
                attraction.status === "Safe"
            ...
            {/* Name below marker */}
            <div className="absolute top-7 left-1/2 -translate-x-1/2 text-xs font-semibold text-gray-700">
              {attraction.name}
            </div>

            {/* Hover popup */}
            {hoveredAttraction === attraction.id && (
              <Card className="absolute top-12 left-1/2 -translate-x-1/2 w-72 shadow-xl z-10 animate-in fade-in slide-in-from-top-2 duration-200">
                <CardContent className="p-4 space-y-3">
                  <h3 className="font-bold text-lg">{attraction.name}</h3>
                  <div className="text-sm space-y-1">
                    <p>Safety Score: {attraction.safetyScore}/100</p>
                    <p>Distance: {attraction.distance}</p>
                    <p>Travel Time: {attraction.travelTime}</p>
